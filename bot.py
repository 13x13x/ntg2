...
# Auto-response for Amazon links in any message
@app.on_message(filters.private)
async def auto_respond(client, message):
    # Check if the message contains a valid Amazon link
    amazon_link_regex = r'https?://(www\.)?(amazon\.in|amzn\.to)/.*'
    if re.search(amazon_link_regex, message.text):
        ...
        user_id = message.from_user.id
        user = users_collection.find_one({"user_id": user_id})
        amazon_tag = user.get('amazon_tag') if user else None
        ...
        if amazon_tag:
            ...
            try:
                ...
                if product_image_url:
                    ...
                    await message.reply("**Error: No product image found.**")

            except Exception as e:
                await message.reply(f"**Error fetching product details: {e}**")
        else:
            await message.reply("**Please add your Amazon tag in user settings**")
    else:
        await message.reply("**Please send a valid Amazon link!**")
