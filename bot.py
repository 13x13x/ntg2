...
async def get_userinfo_img(user_id, profile_path=None):
    ...
    bg = Image.new("RGB", (1280, 720), (30, 30, 30))  # Base color
    ...
    if profile_path and os.path.exists(profile_path):
        try:
            ...
            # Create a black stroke circle
            stroke_size = 10  # Adjust the stroke size as necessary
            draw_stroke = ImageDraw.Draw(bg)
            draw_stroke.ellipse([(440 - stroke_size, 50 - stroke_size), 
                                  (840 + stroke_size, 450 + stroke_size)], 
                                 outline=(0, 0, 0), width=stroke_size)  # Black stroke
            
            # Paste the resized circular image on the background
    ...
    font_size = 60  # Increase font size for better visibility
    user_id_text = str(user_id).upper()
    
    # Calculate text width and height for centering
    text_width, text_height = img_draw.textsize(user_id_text, font=get_font(font_size, font_path))
    
    # Positioning the user ID text at the bottom center
    text_x = (bg.width - text_width) / 2  # Center horizontally
    text_y = bg.height - text_height - 50  # Position 50 pixels from the bottom

    # Add a subtle shadow effect for readability
    shadow_offset = 2
    img_draw.text(
    ...
    img_draw.text(
        (text_x, text_y),
        text=user_id_text,
        font=get_font(font_size, font_path),  # Use the get_font function
...
async def me(client, message):
    ...
    welcome_text = "**sᴇᴛ/ᴇᴅɪᴛ/ʀᴇᴍᴏᴠᴇ ᴀᴍᴀᴢᴏɴ ᴛᴀɢ & ғᴏᴏᴛᴇʀ**\n\n**ᴜsᴇ ᴛʜᴇ ʙᴜᴛᴛᴏɴ ᴛᴏ ᴍᴀɴᴀɢᴇ ʏᴏᴜʀ sᴇᴛᴛɪɴɢs**"
